...
/*added at 07/07/2022*/
bool 
Matching::filteringMatches(sfm::Correspondences2D2D &unfiltered_matches, sfm::CorrespondenceIndices &unfiltered_indices, sfm::CorrespondenceIndices &matches)
{
	int num_matches = unfiltered_matches.size();
	/* Require at least 8 matches. Check threshold. */
	int const min_matches_thres = std::max(8, this->opts.min_feature_matches);
	if (num_matches < min_matches_thres)
	{
		std::cout << "matches below threshold of " << min_matches_thres << "." << std::endl;
		return false;
	}
	/* Compute fundamental matrix using RANSAC. */
	sfm::RansacFundamental::Result ransac_result;
	...
	if (num_inliers < min_inlier_thres)
	{
		std::cout << "inliers below threshold of " << min_inlier_thres << "." << std::endl;
		return false;
	}

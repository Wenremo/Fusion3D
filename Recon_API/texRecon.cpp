...
void scalingTexModel(tex::Model& model, float s)
{
	...
	/*mvs::Samples samples = model.get_vertices();
...
void texRecon(std::string &in_scene, std::string &in_mesh, std::string &out_prefix, float s)
{
	...
	//if(s==0.0f)
		//g_p3DProgressCallback(90, 90, "Generating texture views.");
	...
	if(s==0.0f)
		g_p3DProgressCallback(90, 92, "Building adjacency graph.");
	...
	{
		//g_p3DProgressCallback(90, 95, "Generating texture patches.");

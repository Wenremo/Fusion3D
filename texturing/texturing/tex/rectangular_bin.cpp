...
RectangularBin::RectangularBin(unsigned int width, unsigned int height)
    ...
    rects.push_back(myRect<int>(0, 0, width, height));
}

bool RectangularBin::insert(myRect<int> * rect) {
    /* The best score is 0 so we initialize with the worst. */
    unsigned int best_score = width * height;
    std::list<myRect<int> >::iterator best_rect_it = rects.end();
    std::list<myRect<int> >::iterator it = rects.begin();
    for (; it != rects.end(); ++it) {
        myRect<int> free_rect = *it;
    ...
    if (best_rect_it != rects.end()) {
        myRect<int> best_rect(&(*best_rect_it));
        ...
        /* Decide split axis. */
        myRect<int> hsplit_top(best_rect.min_x, rect->max_y, best_rect.max_x, best_rect.max_y);
        myRect<int> hsplit_bottom(rect->max_x, best_rect.min_y, best_rect.max_x, rect->max_y);
        myRect<int> vsplit_left(best_rect.min_x, rect->max_y, rect->max_x, best_rect.max_y);
        myRect<int> vsplit_right(rect->max_x, best_rect.min_y, best_rect.max_x, best_rect.max_y);
